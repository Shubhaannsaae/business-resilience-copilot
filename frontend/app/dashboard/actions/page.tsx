...
export default function ActionsPage() {
  ...
  async function generate() {
    setLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE}/api/actions/generate?snapshot_id=${snapshotId}`, {
        method: "POST",
        headers: { "X-API-Key": process.env.NEXT_PUBLIC_API_KEY as string }
      });
      if (!res.ok) throw new Error(await res.text());
  ...
  return (
    <main className="p-6 max-w-7xl mx-auto space-y-8 animate-fadeIn">
      <div className="bg-secondary p-8 rounded-xl border border-highlight shadow-2xl">
        <h2 className="text-3xl font-bold text-white mb-2">Generate Actionable Plan</h2>
        <p className="text-light-accent mb-8">Enter a snapshot ID to generate a customized action plan.</p>
        ...
        <div className="flex items-start gap-8">
          ...
          <div className="w-2/3">
            {items && <ActionChecklist items={items} />}
            {!items && !loading && (
              <div className="text-center py-20">
                <Icon path="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" className="text-gray-600 w-16 h-16 mx-auto mb-4" />
                <p className="text-lg text-light-accent">Enter a snapshot ID and click Generate to create an action plan.</p>
              </div>
            )}
          </div>
        </div>
